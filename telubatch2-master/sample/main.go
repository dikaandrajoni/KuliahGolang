...
func createCustomer(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		stmt, err := db.Prepare("INSERT INTO customers (CustomerID,CompanyName,ContactName,ContactTitle,Address,City,Country,Phone) VALUES (?,?,?,?,?,?,?,?)")

		_, err = stmt.Exec(CustomerID, CompanyName, ContactName, ContactTitle, Address, City, Country, Phone)
...
func updateCustomer(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "PUT" {
		...
		stmt, err := db.Prepare("UPDATE customers SET CompanyName = ?, ContactName = ?, ContactTitle = ?, Address = ?, City = ?, Country = ?, Phone = ? WHERE CustomerID = ?")

		_, err = stmt.Exec(newCompanyName, newContactName, newContactTitle, newAddress, newCity, newCountry, newPhone, CustomerID)

		if err != nil {
			fmt.Fprintf(w, "Data not found or Request error")
		}

		fmt.Fprintf(w, "Customer with CustomerID = %s Berhasil di updated", CustomerID)
	}
}
...
func deleteCustomer(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(w, "Customer dengan ID = %s Berhasil di hapus", customerID)
}
